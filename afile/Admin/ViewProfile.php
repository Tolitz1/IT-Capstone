...
<div class="col-md-10" style="position:absolute; left:18%; width: 80%;">
    <!-- Table -->
    <p class="bg-dark text-white p-3" style="border-radius: 2px; font-size:20px;">Workers Approval</p>
    <?php
        $sql ="SELECT * FROM workerslogin_tb WHERE w_status = 'pending' ORDER BY w_login_id DESC";
    ...
    <table class="table">
        ...
        <?php while($row = $result->fetch_assoc()){ 
            ...
            echo '<td>'.$row['w_number'].'</td>';
            echo '<td>'.$row['w_municipality'].'</td>';
            echo '<td>'.$row['w_role'].'</td>';
        
            echo '<td>';
            echo '
            <form action="file.php" method="POST" class="d-inline">
...
<input type="submit" class="deny" name="deny" value="Deny"/>
      ...
      if(isset($_REQUEST['delete'])){
        $sql = "DELETE FROM workerslogin_tb WHERE w_login_id = {$_REQUEST['id']}";
...
<?php
    ...
    $w_login_id = $_POST['w_login_id'];

    $select = "UPDATE workerslogin_tb SET w_status = 'approved' WHERE w_login_id = '$w_login_id'";
...
//WAPPROVAL  /* dharlynaquitania20@gmail.com */ /* "kentalmira23@gmail.com */
...
if(isset($_POST['deny'])){
    ...
    $select = "DELETE FROM workerslogin_tb WHERE w_login_id = '$w_login_id'";
    $result = mysqli_query($conn, $select);
...
//WDenial /* dharlynaquitania20@gmail.com */ /* "kentalmira23@gmail.com */
        $mail->addAddress ("kentalmira23@gmail.com");
